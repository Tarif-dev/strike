...
// Usage: node test-fantasy-points.js [path-to-scorecard-json]
...
try {
  ...
  // Parse the scorecard data
  console.log('Parsing JSON data...');
  const scorecardData = JSON.parse(fileContent);
...
for (const player of sortedPlayers) {
  ...
  if (performance.runs !== undefined) {
    console.log(`  Batting: ${performance.runs || 0} runs off ${performance.balls || 0} balls`);
    ...
    if (performance.strikeRate) {
      console.log(`  Strike Rate: ${performance.strikeRate.toFixed(2)}`);
    }
  }
  
  if (performance.wickets !== undefined) {
    console.log(`  Bowling: ${performance.wickets || 0} wickets for ${performance.bowlingRuns || 0} runs in ${performance.overs || '0'} overs`);
    if (performance.maidens) {
  ...
  if (performance.isCaptain) {
    console.log(`  Role: Captain (points multiplied by ${SCORING_RULES.CAPTAIN_MULTIPLIER})`);
  } else if (performance.isViceCaptain) {
    console.log(`  Role: Vice Captain (points multiplied by ${SCORING_RULES.VICE_CAPTAIN_MULTIPLIER})`);
  } else if (performance.isKeeper) {
    console.log(`  Role: Wicket Keeper`);
  }
}
