...
const CreateMatch = () => {
  ...
  const { publicKey, connected, connecting, disconnect, select, sendTransaction } = wallet;
    ...
    if (!connected && !connecting) {
      ...
      if (wallet.wallet) {
        try {
          ...
          setWalletError("Failed to reconnect wallet. Please connect manually.");
          return false;
        }
      } else {
...
const onSubmit = async (values: CreateMatchFormValues) => {
  ...
  if (!await ensureWalletConnection()) {
    toast({
      ...
      description: walletError || "Please connect your Solana wallet to create a match",
    });
    return;
  }
  ...
  try {
    ...
    const unixRegistrationEndTime = Math.floor(registrationEndTime.getTime() / 1000);
...
try {
  ...
  if (simulationResult.err) {
    ...
    throw new Error(`Simulation failed: ${JSON.stringify(simulationResult.err)}`);
  } else {
  ...
  throw new Error(`Transaction simulation failed: ${simError.message}`);
      ...
      setTxStatus(`Transaction sent! Signature: ${signature.substring(0, 8)}...`);
      ...
      if (confirmationStatus.value.err) {
        throw new Error(`Transaction failed: ${JSON.stringify(confirmationStatus.value.err)}`);
      }
      ...
      if (txError.message?.includes("User rejected")) {
        ...
        throw new Error("You declined the transaction in your wallet. Please try again.");
      } else {
    ...
    toast({
      ...
      description: error instanceof Error ? handleBlockchainError(error) : "An unknown error occurred",
    });
  } finally {
...
const handleBlockchainError = (error) => {
  ...
  if (error.message?.includes("account not found") || error.message?.includes("0x7")) {
  ...
  if (error.message?.includes("Program error:") || error.message?.includes("Program log:")) {
  ...
  return (
    <PageContainer>
      ...
      <div className="flex items-center justify-between px-4 py-2 bg-cricket-dark-green">
        <h1 className="text-lg font-bold text-white">Create Match</h1>
        <WalletMultiButton className="bg-cricket-lime text-cricket-dark-green hover:bg-cricket-lime/90" />
      </div>
  ...
  <div className="rounded-md border border-red-600/20 bg-red-600/10 p-4 mt-4">
    <div className="flex items-center">
      ...
      <div className="ml-3">
        <h3 className="text-sm font-medium text-red-500">Wallet Connection Error</h3>
        ...
        <div className="mt-2">
          <Button
            ...
            onClick={() => {
              if (wallet.wallet) {
                disconnect().then(() => {
                  setTimeout(() => select(wallet.wallet.adapter.name), 500);
                });
              } else {
  ...
  <div className="rounded-md border border-yellow-600/20 bg-yellow-600/10 p-4 mt-4">
    <div className="flex items-center">
      ...
      <div className="ml-3">
        <h3 className="text-sm font-medium text-yellow-500">Blockchain Transaction Status</h3>
...
{txComplete && (
  <div className="rounded-md border border-green-600/20 bg-green-600/10 p-4 mt-4">
    <div className="flex">
      ...
      <div className="ml-3">
        <h3 className="text-sm font-medium text-green-500">Transaction Complete</h3>
        <div className="mt-2 text-sm text-green-400/80">
          <p>Match has been created successfully on the blockchain.</p>
          <p className="mt-1 font-mono text-xs">
            Tx: {txSignature.substring(0, 8)}...{txSignature.substring(txSignature.length - 8)}
          </p>
        </div>
        <div className="mt-2">
          <Button
